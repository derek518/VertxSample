<template>
  <el-form ref="form" :class="{'form-flex':schema.length>2}" class="com-form" label-width="100px">
    <slot name="title" />
    <el-form-item v-for="(field, index) in schema" v-show="handleControl(field.controlBy)" :key="index" :label="field.label" :required="field.required" :error="field.error" :style="{width: field.width}" class="schema-form-item">
      <component
